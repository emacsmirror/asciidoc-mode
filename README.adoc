...
* Cross-reference and link navigation -- follow `<<id>>`/`xref:` to their anchors (explicit or auto-generated section ids) and open URLs, via `RET`, mouse, `C-c C-o`, or `xref` (`M-.` / `M-,` with history)
