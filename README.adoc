...
* Cross-reference and link navigation -- follow `<<id>>`/`xref:` to their anchors (explicit ids, auto-generated section ids, or natural-title refs) and open URLs, via `RET`, mouse, `C-c C-o`, or `xref` (`M-.` / `M-,` with history); completion-at-point offers ids and titles while typing `<<`
