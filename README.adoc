...
* Syntax highlighting via tree-sitter (headings, inline markup, blocks, lists, tables, admonitions, macros, attributes)
* Native source-block highlighting -- `[source,lang]` blocks are fontified with the language's own major mode (toggle and size limit via `asciidoc-fontify-code-blocks-natively`)
...
* Structural editing -- promote/demote headings (`M-<left>` / `M-<right>`) and move subtrees up/down (`M-<up>` / `M-<down>`)
...
I've been maintaining https://github.com/bbatsov/adoc-mode[adoc-mode] since 2022, but over time I realized I rarely used many of its advanced features (e.g. inline image preview, interactive markup commands, tempo templates). I wanted something simpler.
